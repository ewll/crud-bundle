...
class DoctrineSource implements SourceInterface
{
    ...
    {
        throw new \RuntimeException('Method is not realized');
    }
    ...
    public function findList(string $entityClassName, array $conditions, int $page, int $itemsPerPage, array $sort): ItemsList
    {
        $repository = $this->entityManager->getRepository($entityClassName);
        $items = $repository->findAll();//@TODO
        $total = count($items);
